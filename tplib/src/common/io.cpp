...
size_t format_base(const char* const format, size_t format_size, int_format_settings* const settings)
{
    ...
    if((format[format_offset] == 'b' || format[format_offset] == 'B') && settings->base == 10)
    {    
...
offsets_struct format_int(const char* const format, size_t format_size, char* const buffer, const size_t buffer_size, int to_format)
{
    ...
    format_offset += format_base(format + format_offset, format_size - format_offset, &int_format_settings);
...
//        // b - Small letters
//        // B - Capital letters
//        if((format[i] == 'b' || format[i] == 'B') && settings.base == 10)
//        {    
//            if(format[i] == 'B')
//            {
//                settings.capitalized = true;
//            }
//            i++; // Move after modifier char.
//            str_num_struct found_base = str_to_num(format + i, format_size - i); // Turn base to number.
//            int new_base = found_base.number;
//            
//            if(found_base.status != -1 && new_base >= 2 && new_base <= BASE_MAX)
//            {
//                settings.base = new_base;
//                i += found_base.num_digits;
//                if(settings.base == 2)
//                {
//                    settings.big_endian = false;
//                } // Base 2 is usually written with little endian (opposite of usual).
...
//            }
        ...
        //format_offset = i;
    //}
    return {to_str(to_format, buffer, buffer_size, settings.base, settings.capitalized, settings.big_endian, ',', 3), format_offset};
}
